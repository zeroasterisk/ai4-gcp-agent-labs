...
queries at runtime through its skill toolset, so nothing reads a local
SKILL.md. Registering, listing and deleting skills is handled by the
SkillAdmin in `skills.py`. The ops MCP server runs as a local stdio
subprocess. The graph is driven by an in-process runner in
`runtime/local_runner.py`. Sessions and memory are kept in memory by the
harness. Generated code runs in this process.
"""
...
def build_root_agent(
    ...
    stdio subprocess, the runbook skill from the managed Skill Registry, and
    memory recall. `analyze` delegates any arithmetic to an inner agent
    that runs Python. `report` presents the findings and persists memories.
