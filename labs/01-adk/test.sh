...
python -m py_compile run_local.py mcp_server/server.py $(find src/nimbus -name '*.py')
...
if [[ "${RUN_LIVE:-0}" == "1" ]]; then
  ...
  echo "Running the live tests against ${GOOGLE_CLOUD_PROJECT}"
  python -m pytest tests/test_live.py
else
  echo "Skipping the live tests. Set RUN_LIVE=1 to enable them."
fi
