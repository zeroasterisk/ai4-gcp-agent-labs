...
schemas, the skills and the Workflow wiring. The `harness` package holds the
capability subsystems for sessions and memory. The `runtime` package is where
the graph runs, which here is the local process.

The ops tools themselves live in the separate `mcp_server/` package, which is
deployed to Cloud Run. Skills and the MCP server are managed services in this
lab, while sessions, memory and code execution run locally.
"""
