...
`agent` package holds the graph, meaning the config, the prompts, the skills
and the Workflow wiring. The `harness` package holds the capability
subsystems for sessions and memory. The `runtime` package is where the graph
runs, which here is the managed Agent Runtime, driven by deploy.py and
client.py.

Skills and the MCP server are managed services, and the graph is deployed to
the managed Agent Runtime. Sessions, memory and code execution run locally.
"""
