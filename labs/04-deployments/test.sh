...
# RUN_LIVE is set to 1 and deploy.py has already run.
set -euo pipefail
cd "$(dirname "$0")"
...
python -m py_compile deploy.py query_deployed.py delete_deployed.py skill_admin.py mcp_server/*.py $(find src/nimbus -name '*.py')
...
if [[ "${RUN_LIVE:-0}" == "1" ]]; then
  ...
  echo "Querying the deployed agent"
  python -m pytest tests/test_live.py
else
  echo "Skipping the live tests. Set RUN_LIVE=1 once deploy.py has run."
fi
